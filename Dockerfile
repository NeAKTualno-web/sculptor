...
ENTRYPOINT ["sh", "-c", "echo '=== SECRETS CONTENTS ===' && ls -la /etc/secrets/ || true; cp /etc/secrets/*config*.toml /app/config.toml 2>/dev/null || cp /etc/secrets/* /app/config.toml 2>/dev/null || true; ./sculptor"]
